...
const [ state,  setState ] = useState({
  ...
  imagesToShow:[]
    ...
    useEffect(()=>{
      ...
      setState((prevState)=>({...prevState, imagesToShow:images, isLoading: false}));
  ...
  const handleOpenRemoveItem = (item)=>{
    setState((prevState)=>({...prevState, openRemoveItem:true, currentItem: item}));
  }

  const handleMoreImages =()=>{
    setState((prevState)=>({...prevState, endIndex: state.endIndex + IMAGES_PER_LOAD}));
  }

  const handleRemove=()=>{
    const imagesFiltered = state.imagesToShow.filter((item)=>item.id !== state.currentItem?.id)
    setState((prevState)=>({...prevState, imagesToShow: imagesFiltered, openRemoveItem:false}));
  }
